"""Fig 4B rerun v2 with Scheme 1 wedge-restricted polar metrics.

Extends fig4b_pipeline's metric set with:
  * CORRECTLY-ALIGNED pattern extremes (extracted from shifted_template
    indexed in its own frame, so the points sit on the orange contour).
  * UPPER wedge from the pattern-bottom extremum through left/right extremes
    (sweeps up toward the arch).
  * Polar coordinate system anchored at the wedge apex (= pattern bottom).
    r = distance (µm) from wedge apex, restricted to pixels inside the wedge.
  * 1D r-profile + moments + quantiles + Gini/entropy/KS.
  * Wedge-restricted 2D moment tensor.
  * Single-scalar CoM-r (intensity-weighted mean r within the wedge).

All prior metrics from fig4b_pipeline.process_cell are preserved so we can
diff v1 vs v2 outputs.

Output: replication/overnight_fig4b_v2_out/
"""
...
from replication.metric_pipeline import (  # noqa: E402
    _bg_subtracted, _gini, _com_offset_um, _apical_fraction, _weighted_mean,
)
...
DEFAULT_OUT = REPO / "replication" / "overnight_fig4b_v2_out"

WEDGE_R_MAX_UM = 60.0
WEDGE_R_STEP_UM = 1.0


# -------- Scheme 1 wedge helpers (correctly-aligned) --------

def _pattern_extremes_correct(shifted_template, max_coords):
    """Extract pattern extremes from the CORRECTLY-aligned pattern mask
    (sliced in shifted_template's own frame, so the extremes sit on the
    orange contour when overlaid on the crop)."""
    ...
    return ext, pattern_mask


def _upper_wedge_mask(shape, origin_yx, left_pt, right_pt):
    """Angular wedge from origin_yx through left and right points, containing
    the 'up' direction (image-coord negative y). Returns (mask, opening_rad)."""
    H, W = shape
    Y, X = np.mgrid[:H, :W]
    ...
    return mask, float(opening)


def _wedge_polar_metrics(intensity, wedge_apex_yx, wedge_mask, pitch_um,
                         r_max_um=WEDGE_R_MAX_UM, r_step_um=WEDGE_R_STEP_UM):
    """Scheme 1 metrics: polar origin = wedge apex, r = distance from apex.
    Restricted to pixels inside the wedge."""
    H, W = intensity.shape
    Y, X = np.mgrid[:H, :W]
    dy = (Y - wedge_apex_yx[0]) * pitch_um
    dx = (X - wedge_apex_yx[1]) * pitch_um
    r = np.hypot(dy, dx)

    I_w = intensity * wedge_mask
    total_in = float(I_w.sum())
    total_all = float(intensity.sum())
    if total_all <= 0:
        return {}
    out: dict = {"wedge_frac_pct": 100.0 * total_in / total_all}
    if total_in <= 0:
        return out

    # 1D r-profile (1 µm bins out to r_max_um)
    edges = np.arange(0, r_max_um + r_step_um, r_step_um)
    n_bins = len(edges) - 1
    pct = np.full(n_bins, np.nan)
    vol_arc = np.zeros(n_bins)  # expected area per shell assuming uniform-within-wedge
    for i, (lo, hi) in enumerate(zip(edges[:-1], edges[1:])):
        m = (r >= lo) & (r < hi) & wedge_mask
        pct[i] = (m * intensity).sum() / total_in * 100
        vol_arc[i] = m.sum()
        out[f"wedge_r_{int(lo):02d}_{int(hi):02d}um_pct"] = float(pct[i])

    # Moments of r within the wedge
    I_flat = I_w.ravel()
    r_flat = r.ravel()
    w = float(I_flat.sum())
    mean_r = float((I_flat * r_flat).sum() / w)
    var_r = float((I_flat * (r_flat - mean_r) ** 2).sum() / w)
    sd_r = float(np.sqrt(max(0.0, var_r)))
    out["wedge_r_mean_um"] = mean_r
    out["wedge_r_sd_um"] = sd_r
    if sd_r > 0:
        out["wedge_r_skew"] = float((I_flat * ((r_flat - mean_r) / sd_r) ** 3).sum() / w)
        out["wedge_r_kurt"] = float((I_flat * ((r_flat - mean_r) / sd_r) ** 4).sum() / w - 3)
    else:
        out["wedge_r_skew"] = np.nan
        out["wedge_r_kurt"] = np.nan
    ...
    for q in (0.10, 0.25, 0.50, 0.75, 0.90):
        out[f"wedge_r_q{int(q*100):02d}_um"] = float(rs[np.searchsorted(cum, q)])

    # Gini / entropy / KS of r-profile
    out["wedge_r_gini"] = _gini_of_bins(pct)
    out["wedge_r_entropy"] = _entropy_of_bins(pct)
    out["wedge_r_ks_vs_uniform"] = _ks_vs_uniform(pct, vol_arc)

    # r-band fractions (perinuclear/peripheral bands along cell axis)
    for label, lo, hi in [("r_0_20um_frac_pct", 0, 20),
                          ("r_20_35um_frac_pct", 20, 35),
                          ("r_35_55um_frac_pct", 35, 55)]:
        m = (r >= lo) & (r < hi) & wedge_mask
        out[f"wedge_{label}"] = float((m * intensity).sum() / total_in * 100)

    # Wedge-restricted moment tensor around wedge apex
    syy = float((I_w * dy * dy).sum() / w) if w > 0 else np.nan
    sxx = float((I_w * dx * dx).sum() / w) if w > 0 else np.nan
    sxy = float((I_w * dy * dx).sum() / w) if w > 0 else np.nan
    if np.isfinite(syy) and np.isfinite(sxx) and np.isfinite(sxy):
        cov = np.array([[syy, sxy], [sxy, sxx]])
        eigvals, _ = np.linalg.eigh(cov)
        lam_min, lam_max = float(eigvals[0]), float(eigvals[-1])
        out["wedge_mt_apex_lam_max_um2"] = lam_max
        out["wedge_mt_apex_lam_min_um2"] = lam_min
        out["wedge_mt_apex_elongation"] = float(np.sqrt(
            max(lam_max, 1e-12) / max(lam_min, 1e-12)))

    return out


# -------- Extended per-cell routine --------
...
def process_cell(img_path, *, template_hat, template) -> dict:
    ...
    # Legacy pattern_mask_big (wrong-frame — retained so the v1 metric columns
    # match the previous run for diff comparison).
    pattern_mask_big = shifted_template[y_start:y_end, x_start:x_end] > 0
    pattern_com = center_of_mass(pattern_mask_big)

    # CORRECTLY-ALIGNED pattern mask + extremes — used for the new wedge.
    ext, pattern_mask_correct = _pattern_extremes_correct(shifted_template, max_coords)
    pattern_com_correct = center_of_mass(pattern_mask_correct)
    ...
    wedge_mask_img, wedge_opening_rad = _upper_wedge_mask(
        nuc.shape, wedge_apex, ext["left"], ext["right"])
    ...
    metrics: dict = {
        ...
        **seg_quality,
        "wedge_opening_deg": float(np.degrees(wedge_opening_rad)),
        "wedge_px_fraction": float(wedge_mask_img.sum() / wedge_mask_img.size),
    }
    # Pattern extreme positions in µm from nucleus CoM (diagnostic)
    ...
    for proj_name, proj_xr in [("zsum", cropped_zsum), ("maxip", cropped_zmax)]:
        ...
        # ---- EXISTING v1 metric set (preserved for diff) ----
        ...
        # ---- NEW: Scheme 1 wedge-restricted polar metrics ----
        wp = _wedge_polar_metrics(I, wedge_apex, wedge_mask_img, pitch_um)
        for k, v in wp.items():
...
# -------- Orchestration (parallel of fig4b_pipeline.run) --------
...
def run(root: pathlib.Path, out_root: pathlib.Path) -> int:
    ...
    print(f"[fig4b_v2_pipeline] {len(cells)} cells to process under {root}", flush=True)
    ...
    for well_dir, recs in records_by_well.items():
        ...
        print(f"[fig4b_v2_pipeline] wrote {out_csv} ({len(recs)} cells)", flush=True)
    ...
    if all_recs:
        ...
        print(f"[fig4b_v2_pipeline] wrote {combined} ({len(all_recs)} cells total)", flush=True)
    return 0
