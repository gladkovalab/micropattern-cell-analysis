"""Exploratory: % mitochondria in the 20–30 µm radial bin per cell.

For each cell:
  bin_20_30 = sum(wedge_r_20_21um_pct .. wedge_r_29_30um_pct)

This is a single 10-µm slab partway down the wedge — a "mid-zone" intensity
fraction. We plot per-condition strip plots for all 6 comparison sheets in a
single 2x3 figure with nested-ANOVA + Šídák brackets, same statistics as
plot_metrics.py.
"""
...
METRIC_NAME = "wedge_r_20_30um_pct"
METRIC_LABEL = "% mito intensity\nin 20–30 µm bin"


def add_metric(df: pl.DataFrame) -> pl.DataFrame:
    cols = [f"wedge_r_{i:02d}_{i+1:02d}um_pct" for i in range(20, 30)]
    cols = [c for c in cols if c in df.columns]
    if not cols:
        raise RuntimeError("no wedge_r_NN_NN+1um_pct columns in 20..30 found")
...
def main():
    ...
    ap.add_argument("--out", default="replication/figures_wedge_r_ks/bin_20_30um_all.png")
    args = ap.parse_args()
    ...
    print("=== per-sheet stats: % mito intensity in 20–30 µm bin ===")
    ...
    fig.suptitle("Wedge-r 20–30 µm slab — per-condition fraction of wedge intensity\n"
